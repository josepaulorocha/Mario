...
let powerUpEffects;
let levelRegions;
let lastOpenScreen = 'game';
let allCharactersData;
let bonusEventsHeap;
// --- CONTROLE DE PERSONAGENS ---
const characterRotation = new DoublyCircularLinkedList();
...
// --- FUNÇÕES PRINCIPAIS ---
function startGameWith(characterName) {
    if (!allCharactersData) {
        allCharactersData = new HashTable();
    }
    characterRotation.setCurrentByData(characterName);
    characterSelection.style.display = 'none';
    gameContainer.style.display = 'block';
    const filaDeLogs = new FilaDupla();
    const historicoDeAcoes = new Pilha();
    const personagemBase = new Personagem(characterName, filaDeLogs, historicoDeAcoes);
    const enemyDefeatCount = new Dictionary();
...
function initGame() {
    ...
    powerUpEffects = new HashTable();
    powerUpEffects.put('Cogumelo', (p) => { p.vidas += 1; p.log.addBack(`[EFEITO]: ${p.nome} usou um Cogumelo e ganhou uma vida extra!`); });
    powerUpEffects.put('Flor de fogo', (p) => { p.isInvincible = true; p.invincibilityTimer = 300; p.log.addBack(`[EFEITO]: ${p.nome} usou uma Flor de Fogo e está invencível por 5 segundos!`); });
    powerUpEffects.put('Pena', (p) => { p.speedMultiplier = 1.3; p.log.addBack(`[EFEITO]: ${p.nome} usou uma Pena e está mais rápido!`); });
    levelRegions = new LinkedListOfSets();
    const region1Items = new Set(["Cogumelo"]);
    const region2Items = new Set(["Cogumelo", "Flor de fogo"]);
    const region3Items = new Set(["Cogumelo", "Flor de fogo", "Pena"]);
    levelRegions.add(region1Items);
    levelRegions.add(region2Items);
    levelRegions.add(region3Items);
    bonusEventsHeap = new MinHeap((a, b) => {
        if (a.scoreThreshold < b.scoreThreshold)
...
function updateDataStructuresScreen(characterName) {
    ...
    data.log.toArray().forEach(log => {
        ...
        li.textContent = log;
        logQueueList.appendChild(li);
    });
    data.historico.toArray().reverse().forEach(action => {
        const li = document.createElement('li');
        li.textContent = action;
        actionStackList.appendChild(li);
    });
    data.enemyDefeatCount.keyValues().forEach(pair => {
        const li = document.createElement('li');
        li.textContent = `${pair.key}: ${pair.value}`;
        enemyDictList.appendChild(li);
    });
}
function showPhaseCompleteScreen(message, isGameOver = false) {
...
function update() {
    ...
    if (keys['ArrowRight'] && player.x < LEVEL_WIDTH - player.width)
        player.x += currentSpeed;
    ...
    powerUpBlocks.forEach(block => {
        ...
        if (!block.hit && player.velocityY < 0 &&
            ...
            if (levelRegions.getHead()) {
                ...
                else if ((_a = levelRegions.getHead().next) === null || _a === void 0 ? void 0 : _a.next) {
...
function calculateVarietyBonus(player) {
    var _a;
    const enemyData = (_a = allCharactersData.get(player.nome)) === null || _a === void 0 ? void 0 : _a.enemyDefeatCount;
...
window.addEventListener('keydown', (e) => {
    if (!isPaused)
        keys[e.code] = true;
    if (player && player.historico && !isPaused)
        player.historico.push(`[TECLA DOWN]: ${e.code}`);
...
window.addEventListener('keyup', (e) => {
    if (!isPaused)
        keys[e.code] = false;
    if (player && player.historico && !isPaused)
        player.historico.push(`[TECLA UP]: ${e.code}`);
});
